...
if(keyboard_check_pressed(vk_left)){
	...
	switch(selected_option){
		case 1: if(points_available >0 && global.checkpasses >= 1 && global.checkpasses < 10)		{global.checkpasses++;		points_available--;}break;
		case 0: if(points_available < max_points_available && global.checkpasses > 1)				{global.checkpasses--;		points_available++;}break;
		case 3: if(points_available >0 && global.eavesdropping >= 1 && global.eavesdropping < 10)	{global.eavesdropping++;	points_available--;}break;
		case 2:	if(points_available < max_points_available && global.eavesdropping > 1)				{global.eavesdropping--;	points_available++;}break;
		case 5:	if(points_available >0 && global.readlips >= 1 && global.readlips < 10)				{global.readlips++;			points_available--;}break;
...
if((room==rm_1 or room==rm_4) and global.alpha <= 0){
	room_goto_next();
